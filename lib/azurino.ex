defmodule Azurino do
  @moduledoc "List Azure container blobs via SAS URL"

  def list_container(container_sas_url) when is_binary(container_sas_url) do
    list_url =
  ...
  def list_folder(container_sas_url, folder_path) when is_binary(container_sas_url) and is_binary(folder_path) do
    # Ensure folder_path ends with / for proper prefix matching
    normalized_folder =
      if String.ends_with?(folder_path, "/") do
        folder_path
      else
        folder_path <> "/"
      end

    # Build URL with prefix parameter
    base_url =
      if String.contains?(container_sas_url, "?") do
    ...
    list_url = base_url <> URI.encode(normalized_folder)
  ...
  def upload(container_sas_url, remote_folder, local_file_path)
      when is_binary(container_sas_url) and is_binary(remote_folder) and is_binary(local_file_path) do
